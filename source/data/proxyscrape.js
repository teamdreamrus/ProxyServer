...
let geoIpLL = require("geoip-lite");
        ...
        if (body) {
          try {
            ...
            out[info.country].push({
              ...
              ip: host,
              port: port,
              timeSpentСhecking: new Date() - timeStart,
              ll: ll
            });
          } catch(e) {
          }
...
module.exports.start = async function() {
  await new Promise((resolve) => {
    download('https://api.proxyscrape.com?request=getproxies&proxytype=http&timeout=10000&country=all&ssl=yes&anonymity=all', './list_proxyscrape.txt', resolve);
  });
  await new Promise((resolve) => {
  ...
  proxy = proxy.concat(fs.readFileSync('./list_p-l_ru.txt', 'utf8').toString().split("\r\n"));
  ...
  for (let i = 0; i < proxy.length; i++) {
    if (proxy[i].split(':').length === 2 && !proxy[i].split(':')[1].includes('.')) {
      // tut country and LL
      let geo = geoIpLL.lookup(proxy[i].split(':')[0]) || {
        country: 'UNKNOWN'
      };
      // proxy[i].ll = geo.ll || [null, null];
      if (geo && geo.country) {
        if (geo.country == 'US'
          || geo.country == 'FR'
          || geo.country == 'NL'
          ...
          promises.push(new Promise(resolve => {
            ...
            return check(proxy[i], geo.ll, resolve);
          }));
          }
      }
    }
  ...
  fs.writeFileSync('config.json', JSON.stringify(outComplete), 'utf8');
  console.log('complete');
  this.list = outComplete;
}
